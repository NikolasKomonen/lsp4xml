...
public class FilePathCompletionParticipant extends CompletionParticipantAdapter {
	...
	public void onAttributeValue(String valuePrefix, Range fullRange, boolean addQuotes, ICompletionRequest request,
		...
		int parsedAttributeStartOffset = StringUtils.getOffsetAfterWhitespaceInfront(fullAttributeValue, completionOffset - documentStartOffset) + documentStartOffset; // first character of URI
		
	
		Position startValue = xmlDocument.positionAt(parsedAttributeStartOffset);
		int endOffsetFromCompletionOffset = StringUtils.getNextWhitespaceOffsetFromGivenOffset(text, completionOffset);

		int actualEndOffset = completionOffset + endOffsetFromCompletionOffset;
		String attributePath = text.substring(parsedAttributeStartOffset, completionOffset);
		Position endValue = xmlDocument.positionAt(actualEndOffset);
