...
public class StringUtils {
	...
	/**
	 * Returns the initial starting whitespaces of the given line text.
	 * 
	 * Once a non-whitespace character is found it stops.
	 * 
	 * @param lineText
	...
	/**
	 * Returns the amount of character positions after the given startOffset
	 * until a whitespace character is found.
	 * 
	 * The character at startOffset + result will be the first whitespace character
	 * that is came across.
	 * 
	 * @param lineText
	 * @param startOffset
	 * @return
	 */
	public static int getNextWhitespaceOffsetFromGivenOffset(String lineText, int startOffset) {
		
		if(startOffset > 0) {
			lineText = lineText.substring(startOffset);
		}
		if(isEmpty(lineText)) {
			return -1;
		}
		...
		for (i = 0; i < lineText.length(); i++) {
			...
			if (Character.isWhitespace(c) || isQuote(c)) {
				return i;
			}
		}
		return i;
	 ...
	 */
	public static int getOffsetAfterWhitespaceInfront(String text, int endOffset) {
